...
public class MainFragment extends Fragment
   ...
   /* if (SDK_INT >= Q && ArraysKt.any(ANDROID_DATA_DIRS.toArray(), providedPath::contains)) {
      openMode = loadPathInQ(actualPath, providedPath, providedOpenMode);
    }*/


        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
            String[] dataDirsArray = ANDROID_DATA_DIRS.toArray(new String[0]);
            boolean containsPath = false;
            for (String path : dataDirsArray) {
                if (providedPath.contains(path)) {
                    containsPath = true;
                    break;
                }
            }
            ...
            if (!mainFragmentViewModel.getRetainSearchTask()) {
                ...
                if (requireMainActivity().getListItemSelected()) {
                    ...
                    if (OpenMode.SMB.equals(mainFragmentViewModel.getOpenMode())) {
                        ...
                        String parentPath = currentFile.getParent(getContext());
                        if (FileProperties.ANDROID_DEVICE_DATA_DIRS.contains(parentPath)) {
                            loadlist(parentPath, false, OpenMode.ANDROID_DATA, false);
                        } else {
                            loadlist(currentFile.getParent(getContext()),
                                    true,
                                    mainFragmentViewModel.getOpenMode(),
                                    false);
                        }
